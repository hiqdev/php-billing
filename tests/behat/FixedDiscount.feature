Feature: Fixed discount
    ...
    Scenario Outline: absolute fixed discount without limits with reason
        Given formula is discount.fixed('25 USD').reason('TEST_COMMENT')
         When action date is <date>
         Then first charge is <first>
          And second charge is <second>
        Examples:
            | date       | first           | second                               |
            | 2018-07-01 | monthly 100 USD | discount -50 USD reason TEST_COMMENT |
            | 2018-08-01 | monthly 100 USD | discount -50 USD reason TEST_COMMENT |
            | 2018-09-01 | monthly 100 USD | discount -50 USD reason TEST_COMMENT |
            | 2028-11-11 | monthly 100 USD | discount -50 USD reason TEST_COMMENT |

    Scenario Outline: absolute fixed discount with since date
        Given formula is discount.since('08.2018').fixed('25 USD')
         When action date is <date>
         Then first charge is <first>
          And second charge is <second>
        Examples:
