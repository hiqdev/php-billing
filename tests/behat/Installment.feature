Feature: installment
    ...
    Scenario: installment can't have `till`
        Given formula is installment.since('08.2018').till('10.2018')
         Then error is till can not be defined for installment
         When calculating charges

    Scenario Outline: simple installment with reason
        Given formula is installment.since('08.2018').lasts('3 months').reason('TEST')
    ...
    Scenario Outline: simple installment with reason
      Given formula is installment.since('01.2024').lasts('3 months').reason('TEST')
      When action date is <date>
      Then first charge is <first> with <events>
      Examples:
        | date       | first                            | events                     |
    ...
    Scenario Outline: installment will not work when price is zero
          Given server monthly price is 0 USD per item
            And formula is installment.since('08.2018').lasts('3 months').reason('TEST')
           When action date is <date>
           Then first charge is <first> with <events>
        Examples:
          | date       | first                     | events                 |
