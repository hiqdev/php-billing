...
class BillingContext extends BaseContext
{
    ...
    #[\Behat\Step\Given('/price for (\S+) is +(\S+) (\S+) per (\S+) for target (.+)$/')]
    ...
    #[\Behat\Step\Given('/price for (\S+) is +(\S+) (\S+) per (\S+) prepaid (\S+)$/')]
    ...
    #[\Behat\Step\Given('/price for (\S+) is +(\S+) (\S+) per (\S+) prepaid (\S+) for target (\S+)$/')]
    ...
    #[\Behat\Step\Given('/price for (\S+) is +(\S+) (\S+) per (\S+)$/')]
    public function price($type, $price, $currency, $unit)
    {
        return $this->fullPrice(compact('type', 'price', 'currency', 'unit'));
    }

    #[\Behat\Step\Given('/price for (\S+) is +(\S+) (\S+) per 1 (\S+) and (\S+) (\S+) per 2 (\S+) for target (\S+)/')]
    ...
    #[\Behat\Step\Given('/progressive price for (\S+) is +(\S+) (\S+) per (\S+) (\S+) (\S+) (\S+)$/')]
    ...
    #[\Behat\Step\Given('/^remove and recreate tariff plan (\S+)/')]
    public function recreatePlan($plan)
    {
        $this->builder->recreatePlan($plan);
    }

    #[\Behat\Step\Given('/sale target (\S+) by plan (\S+) at (\S+)/')]
    ...
    #[\Behat\Step\When('/^sale close is requested for target "([^"]*)" at "([^"]*)", assuming current time is "([^"]*)"$/')]
    public function saleClose(string $target, string $time, ?string $wallTime): never
    {
        throw new PendingException();
    }

    #[\Behat\Step\Then('/^target "([^"]*)" has exactly (\d+) sale for customer$/')]
    ...
    #[\Behat\Step\Given('/purchase target (\S+) by plan (\S+) at ([-:\w\s]+)$/')]
    ...
    #[\Behat\Step\Given('/^purchase target "([^"]*)" by plan "([^"]*)" at "([^"]*)" with the following initial uses:$/')]
    public function purchaseTargetWithInitialUses(string $target, string $plan, string $time, TableNode $usesTable): void
    {
        $time = $this->prepareTime($time);
        $uses = array_map(static fn(array $row) => [
            'type' => $row['type'],
            'unit' => $row['unit'],
            'amount' => $row['amount'],
    ...
    #[\Behat\Step\Given('/resource consumption for (\S+) is +(\S+) (\S+) for target (\S+) at (.+)$/')]
    ...
    #[\Behat\Step\Given('/perform calculation( at (\S+))?/')]
    ...
    #[\Behat\Step\Given('/bill +for (\S+) is +(\S+) (\S+) per (\S+) (\S+) for target (.+?)( +at (.+))?$/')]
    public function billWithTime($type, $sum, $currency, $quantity, $unit, $target, $at = null, $time = null)
    {
        ...
        Assert::assertEquals(bcmul((string) $sum, 100), $bill->getSum()->getAmount(), "Bill sum mismatch: expected $sum, got {$bill->getSum()->getAmount()}");
        ...
        Assert::assertEquals(strtolower((string) $unit), strtolower($bill->getQuantity()->getUnit()->getName()), "Bill unit mismatch: expected $unit, got {$bill->getQuantity()->getUnit()->getName()}");
        if ($time) {
    ...
    #[\Behat\Step\Given('/bill interval +for (\S+) is +(\S+) (\S+) per (\S+) (\S+) for target (.+?) at (\S+) between (\S+) and (\S+)?$/')]
    ...
    #[\Behat\Step\Given('/bills number is (\d+) for (\S+) for target (.+?)( +at (\S+))?$/')]
    ...
    public function chargeWithTarget($type, $amount, $currency, $quantity, $unit, $target, $at = null, $time = null)
    {
        ...
        Assert::assertEquals(bcmul((string) $amount, 100), (int)$charge->getSum()->getAmount());
        ...
        Assert::assertEquals(strtolower((string) $unit), strtolower($charge->getUsage()->getUnit()->getName()));
    }
    ...
    {
        if ($sum[0] === 's') {
            $sum = round(substr((string) $sum, 1) * $quantity*100)/100;
        }
    ...
    #[\Behat\Step\When('/^tariff plan change is requested for target "([^"]*)" to plan "([^"]*)" at "([^"]*)"$/')]
    ...
    #[\Behat\Step\When('/^tariff plan change is requested for target "([^"]*)" to plan "([^"]*)" at "([^"]*)", assuming current time is "([^"]*)"$/')]
    ...
    #[\Behat\Step\Then('/^target "([^"]*)" is sold to customer by plan "([^"]*)" since "([^"]*)"(?: till "([^"]*)")?$/')]
    ...
    #[\Behat\Step\Then('/^target "([^"]*)" has exactly (\d+) sales for customer$/')]
